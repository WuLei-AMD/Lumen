"""DeepSeek-V4 attention with Lumen linear/norm (no Transformer Engine)."""
...
class DeepSeekV4Attention(MegatronModule):
    """DSV4 sparse MLA attention — Lumen linear/norm, aiter/tilelang kernels."""
    ...
    def __init__(
        ...
        self.wq_a = LumenDuplicatedLinear(
            self.dim,
            self.q_lora_rank,
            config=config,
        ...
        self.wkv = LumenDuplicatedLinear(
            self.dim,
            self.head_dim,
            config=config,
            init_method=config.init_method,
...
def _patch_megatron_no_te() -> None:
    ...
    def _is_te_min_version(version, check_equality=True):
        te_v = mu.get_te_version()
        if te_v is None:
            return False
        if check_equality:
            return te_v >= PkgVersion(version)
        return te_v > PkgVersion(version)

    mu.is_te_min_version = _is_te_min_version
    import megatron.core.optimizer.optimizer_config as oc
...
def get_dsv4_spec(args, config, vp_stage):
    """Megatron custom spec hook (Lumen linear/norm for attention + MoE/MLP).
