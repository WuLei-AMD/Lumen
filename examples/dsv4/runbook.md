...
**p38 上还需**：`~/Lumen`、`~/TileKernels`（与 p14 同步）。
...
| `V4_INDEXER_IMPL` | `aiter` | DSA indexer（默认 aiter，无需 TileLang） |
| `V4_SPARSE_MLA_BACKEND` | `triton` | sparse MLA（aiter triton 后端） |
...
| rollout 数据缺失 | 使用已有 `fake_rollout.pt`，或运行 `tools/gen_fake_rollout_data.py` 生成 |
...
| `NET/IB : Unable to open device mlx5_*` | 容器内 IB 不可用时的 WARN，finetune 曾可 fallback 到 socket |
