...
def patch_dump_grad_norm(megatron_root: str) -> bool:
    """Dump named-param grad-norm membership. Replaces v1 if already patched."""
    import re

    path = os.path.join(megatron_root, "megatron", "core", "optimizer", "optimizer.py")
    if not os.path.isfile(path):
        return False
    with open(path) as f:
        content = f.read()
    if "_DSV4_GN_DUMP_V3" in content:
        return False
    marker = "from .clip_grads import clip_grad_by_total_norm_fp32, count_zeros_fp32, get_grad_norm_fp32\n"
    helper = r'''from .clip_grads import clip_grad_by_total_norm_fp32, count_zeros_fp32, get_grad_norm_fp32

_DSV4_GN_DUMP_V3 = True
_DSV4_GN_DUMPED = False


def _dsv4_gn_bucket(name):
    n = name.replace("module.", "")
    if "shared_expert" in n:
        return "shared_expert"
    if ".experts." in n or "mlp.experts" in n:
        return "expert"
    if any(k in n for k in ("wq_b", "wo_a", "wo_b", "wq_a", "wkv", "attn_sink")):
        return "attn"
    if "indexer" in n or "compressor" in n:
        return "indexer"
    if any(k in n for k in ("output_layer", "word_embeddings", "embedding")):
        return "embed_out"
...
def _dsv4_dump_grad_norm(opt):
    """Named TP-filter dump. Env: DSV4_DUMP_GRAD_NORM=1."""
    import os
    from collections import defaultdict
    from pathlib import Path

    global _DSV4_GN_DUMPED
    if os.environ.get("DSV4_DUMP_GRAD_NORM", "0") != "1" or _DSV4_GN_DUMPED:
        return
    children = list(getattr(opt, "chained_optimizers", None) or [opt])
    ...
    use_decoupled = bool(
        getattr(getattr(children[0], "config", None), "use_precision_aware_optimizer_no_fp8_or_ds_fp8", False)
    )

    def _grad_of(p):
        if use_decoupled and getattr(p, "decoupled_grad", None) is not None:
            return p.decoupled_grad
    ...
    for child in children:
        for chunk in getattr(child, "model_chunks", None) or []:
    ...
    for chunk in chunks:
        for name, param in chunk.named_parameters():
            ...
            main_p = getattr(param, "main_param", None)
            if main_p is not None:
                ptr_to_name[main_p.data_ptr()] = name
    rows = []
    buckets = defaultdict(lambda: {"n": 0, "n_tp": 0, "n_inc": 0, "sq": 0.0, "sq_inc": 0.0, "sq_exc": 0.0})
    sources = []
    for opt_i, child in enumerate(children):
        tp_group = getattr(child, "tp_group", None)
        for pi, param in enumerate(child.get_parameters()):
            name = ptr_to_name.get(param.data_ptr(), f"opt{opt_i}_p{pi}")
            sources.append((name, param, tp_group))
    if not sources:
        for name, param in named_params:
            sources.append((name, param, getattr(children[0], "tp_group", None)))
    for name, param, tp_group in sources:
        grad = _grad_of(param)
        is_tp = bool(getattr(param, "tensor_model_parallel", False))
        is_sp = bool(getattr(param, "sequence_parallel", False))
        allreduce = getattr(param, "allreduce", True)
        inc = False
        gsq = 0.0
        if grad is not None:
            gsq = float(grad.detach().float().pow(2).sum().item())
            inc = param_is_not_shared(param) and tensor_parallel.param_is_not_tensor_parallel_duplicate(
                param, tp_group
            )
        b = _dsv4_gn_bucket(name)
        buckets[b]["n"] += 1
        if is_tp:
            buckets[b]["n_tp"] += 1
        buckets[b]["sq"] += gsq
        if inc:
            buckets[b]["n_inc"] += 1
            buckets[b]["sq_inc"] += gsq
        else:
            buckets[b]["sq_exc"] += gsq
        rows.append((gsq, name, is_tp, is_sp, allreduce, inc, tuple(param.shape)))
    if not any(gsq > 0 for gsq, *_ in rows):
        return
    _DSV4_GN_DUMPED = True
    tp_group = getattr(children[0], "tp_group", None)
    dump_dir = Path(os.environ.get("DSV4_DUMP_GRAD_NORM_DIR", "/tmp/dsv4_gn_dump"))
    try:
        dump_dir.mkdir(parents=True, exist_ok=True)
        out = dump_dir / f"w{world:02d}_pp{pp_rank}_tp{tp_rank}_ep{ep_rank}.txt"
        lines = [
            f"world={world} tp={tp_rank} pp={pp_rank} ep={ep_rank} "
            f"tp_group_rank={tp_group.rank() if tp_group is not None else -1} "
            f"tp_group_size={tp_group.size() if tp_group is not None else -1} "
            f"use_decoupled={use_decoupled} n_named={len(rows)}"
        ]
        for b in sorted(buckets):
            s = buckets[b]
            lines.append(
                f"bucket {b} n={s['n']} n_tp={s['n_tp']} n_inc={s['n_inc']} "
                f"sq={s['sq']:.6e} sq_inc={s['sq_inc']:.6e} sq_exc={s['sq_exc']:.6e}"
            )
        lines.append("TOP_INC")
        for gsq, name, is_tp, is_sp, allreduce, inc, shape in sorted(rows, reverse=True)[:25]:
            if not inc:
                continue
            lines.append(
                f"inc gsq={gsq:.6e} tp={int(is_tp)} sp={int(is_sp)} ar={allreduce} "
                f"shape={shape} {name}"
            )
        lines.append("TOP_EXC")
        for gsq, name, is_tp, is_sp, allreduce, inc, shape in sorted(rows, reverse=True)[:40]:
            if inc:
                continue
            lines.append(
                f"exc gsq={gsq:.6e} tp={int(is_tp)} sp={int(is_sp)} ar={allreduce} "
                f"shape={shape} {name}"
            )
        lines.append("TP_FALSE_DENSE")
        for gsq, name, is_tp, is_sp, allreduce, inc, shape in sorted(rows, reverse=True):
            if is_tp or "experts" in name:
                continue
            if gsq <= 0:
                continue
            lines.append(
                f"notp gsq={gsq:.6e} inc={int(inc)} sp={int(is_sp)} ar={allreduce} "
                f"shape={shape} {name}"
            )
        out.write_text("\n".join(lines) + "\n")
        tsv = dump_dir / f"w{world:02d}_pp{pp_rank}_tp{tp_rank}_ep{ep_rank}.tsv"
        with tsv.open("w") as tf:
            tf.write("gsq\ttp\tsp\tar\tinc\tshape\tname\n")
            for gsq, name, is_tp, is_sp, allreduce, inc, shape in rows:
                tf.write(
                    f"{gsq:.8e}\t{int(is_tp)}\t{int(is_sp)}\t{allreduce}\t{int(inc)}\t{shape}\t{name}\n"
                )
        print(f"[dsv4-gn-dump] wrote {out} world={world} pp={pp_rank} tp={tp_rank}", flush=True)
        for b in sorted(buckets):
            s = buckets[b]
            print(
                f"[dsv4-gn-dump] w={world} pp={pp_rank} tp={tp_rank} bucket={b} "
                f"n={s['n']} n_tp={s['n_tp']} n_inc={s['n_inc']} "
                f"sq={s['sq']:.4e} sq_inc={s['sq_inc']:.4e} sq_exc={s['sq_exc']:.4e}",
                flush=True,
            )
    except Exception as err:
        print(f"[dsv4-gn-dump] failed world={world}: {err}", flush=True)


'''
    if "_dsv4_dump_grad_norm" in content:
        content2, n = re.subn(
            r"_DSV4_GN_DUMPED = False\n\n\ndef _dsv4_dump_grad_norm\(opt\):.*?(?=\n\ndef |\nclass )",
            helper[helper.find("_DSV4_GN_DUMP_V3") :].rstrip() + "\n\n\n",
            content,
            count=1,
            flags=re.S,
        )
        if n != 1:
            return False
        content = content2
        if "_dsv4_dump_grad_norm(self)" not in content:
            return False
        with open(path, "w") as f:
            f.write(content)
        return True
    ...
    old_main = '''    @torch.no_grad()
    def get_grad_norm(self):
        """Compute and return grad norm."""
        grads_for_norm = self.get_main_grads_for_grad_norm()
'''
    new_main = '''    @torch.no_grad()
    def get_grad_norm(self):
        """Compute and return grad norm."""
        _dsv4_dump_grad_norm(self)
        grads_for_norm = self.get_main_grads_for_grad_norm()
'''
    old_chained = '''    @torch.no_grad()
    def get_grad_norm(self):
        if len(self.chained_optimizers) == 1:
'''
    new_chained = '''    @torch.no_grad()
    def get_grad_norm(self):
        _dsv4_dump_grad_norm(self)
        if len(self.chained_optimizers) == 1:
'''
    if old_main not in content or old_chained not in content:
        return False
    content = content.replace(old_main, new_main, 1).replace(old_chained, new_chained, 1)
    with open(path, "w") as f:
        f.write(content)
    return True
...
def main(megatron_root: str) -> None:
    results = {
        ...
        "dump_grad_norm": patch_dump_grad_norm(megatron_root),
    }
